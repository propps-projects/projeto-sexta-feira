...
export function buildServer(adapterMode: AdapterMode = "mcpApps"): McpServer {
  ...
  server.registerTool(
    ...
    async ({ query, limit, lessonNumber }) => {
      ...
      if (!filtered.length) return { content: [{ type: "text", text: `Sem resultados para "${query}".` }] };
  ...
  server.registerTool(
    ...
    async ({ lessonNumber, lessonId, startSec }) => {
      ...
      return { content: [{ type: "text", text: label }, resource] };
    },
  );
  ...
  server.registerTool(
    ...
    async ({ lessonNumber, lessonId, startSec, endSec }) => {
      ...
      if (!segs.length) return { content: [{ type: "text", text: `Sem conteúdo entre ${formatTimestamp(startSec)} e ${formatTimestamp(endSec)} na aula ${lesson.lessonNumber}.` }] };
      const body =
